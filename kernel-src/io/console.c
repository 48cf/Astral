...
static void consolethread() {
	...
	for (;;) {
		...
		if (ev.code == INPUT_KEY_LEFTCTRL || ev.code == INPUT_KEY_RIGHTCTRL) {
			ctrl = ev.value != 0;
			continue;
		} else if (ev.code == INPUT_KEY_LEFTSHIFT || ev.code == INPUT_KEY_RIGHTSHIFT) {
			shift = ev.value != 0;
			continue;
		}
...
INIT_ROUTINE_DEFINE(console, INIT_ROUTINE_FLAGS_NONE, console_init, scheduler, input);
