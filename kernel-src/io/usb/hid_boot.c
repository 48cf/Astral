...
static inline bool check_key(usb_hid_boot_data_t *data, uint8_t keycode) {
	uint32_t index = keycode / 64;
	uint32_t bit = keycode % 64;
	return (data->key_state[index] & ((uint64_t)1 << bit)) != 0;
}

static inline void set_key(usb_hid_boot_data_t *data, uint8_t keycode, bool pressed) {
	uint32_t index = keycode / 64;
	uint32_t bit = keycode % 64;
	if (pressed)
...
int usb_hid_attach_boot(usb_device_t *dev, usb_probe_ctx_t *ctx, void **driver_data) {
	...
	if (interface->bInterfaceProtocol == USB_PROTOCOL_KEYBOARD)
		data->device.keyboard = keyboard_new();
	else if (interface->bInterfaceProtocol == USB_PROTOCOL_MOUSE)
		data->device.mouse = mouse_new();

	data->dev = dev;

	memset(&data->key_state, 0, sizeof(data->key_state));
	memcpy(&data->interface, interface, sizeof(usb_interface_desc_t));
